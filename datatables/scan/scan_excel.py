...
def sheet_min_row(sheet: Worksheet, limit_search: int = 100) -> int:
    ...
    sheet and returns the row index (1-based) of the first row
    containing data.
...
def sheet_field_headers_row(sheet: Worksheet, min_col: int, max_col: int, min_row: int, max_row: int) -> int:
    ...
    for column_index in range(min_col, max_col + 1):
        ...
        if len([cell for cell in column if type(cell) != type('')]) < len(column) and header_row > 0:
            return header_row
        elif len([cell for cell in column if type(cell) == type('')]) == len(column):
            header_types.append(header_type)
        
    if len([h_type for h_type in header_types if h_type == type('')]) == len(header_types):
        return min_row
    else:
        return -1
...
def sheet_hidden_columns(sheet: Worksheet, min_col: int, max_col: int) -> list:
    ...
    -------
    list -> [<col_index: int>]
        Returns a list of 1-based column indices referring to first
...
def sheet_hidden_rows(sheet: Worksheet, min_row: int, max_row: int) -> list:
    ...
    -------
    list -> [<row_index: int>]
        Returns a list of 1-based column indices referring to all hidden
        rows.
    """
    hidden = []
...
def sheet_merged_cells(sheet: Worksheet) -> dict:
    ...
    if len(merged) > 0:
        for cell_range in merged:
            start = coordinate_to_tuple(cell_range.split(':')[0])
            end = coordinate_to_tuple(cell_range.split(':')[1])
            merged_cells[cell_range] = [cell_range.split(':')[0], start, end]
        return merged_cells
    return None
...
def sheet_hierarchical_fields(sheet: Worksheet, min_row: int, header_row: int, min_col: int, max_col: int) -> list:
    ...
    max_col : int
        1-based column index for the last non-empty column in the
        sheet.
